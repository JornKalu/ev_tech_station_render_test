...
async def by_id(db: Session = Depends(get_session), station_id: int = 0):
...
async def by_id(db: Session = Depends(get_session), code: str = None):
...
async def open_slot(fields: OpenSlotModel, db: Session = Depends(get_session)):
...
async def slot(fields: ManageStationSlotModel, db: Session = Depends(get_session)):
...
async def slot_update(fields: ManageSlotStatusModel, db: Session = Depends(get_session)):
    req = manage_slot_status(db=db, station_id=fields.station_id, slot_number=fields.slot_number, status=fields.status)
    return req

@router.post("/process")
async def process(request: Request, db: Session = Depends(get_session)):
...
async def sync(fields: SyncStationModel, db: Session = Depends(get_session)):
    slots = json.loads(fields.slots)
